...
function Table(quantity, table) {
  madeIn = "Russia"; //её область видимости ограничена функцией-Конструктором Table
  ...
  this.checkWhoseTable = function (tableFor) {
    console.log(
      `The ${tableFor} from Ikea made in ${madeIn} and ${(madeIn = "USA")}!`
    );
  };
  const checkIfThereIsASocket = function (yesOrNo) {
    console.log(`the socket is: ${yesOrNo}`);
  }; //область видимости этого функционального выражения ограничена функцией-конструктором Table
...
for (
  let i = 1;
  /*её область видимости ограничена циклом for*/ i < quantityStudents;
  i++
) {
  console.log(i);
};
...
if (quantityStudents < 12) {
  console.log(
    `quantity Students in room is ${(quantityStudents = 7)}` /*её область видимости ограничена модулем Table.js*/
  );
}
...
switch (quantityStudents) {
  case 4:
    ...
    break;
  case 7:
    quantityStudents = "пол группы"; //её область видимости ограничена модулем Table.js
